...
(function(){
  ...
  function ensurePanel(){
    ...
    d.innerHTML =
      '<div class="mhero-panel-in">' +
        ...
        '<label class="mhero-file"><input type="file" id="mhero-file" accept="image/*" hidden><span id="mhero-file-lbl">Escolher imagem (JPG/PNG, at\u00e9 12 MB)</span></label>' +
  ...
  function openPanel(key){
    ...
    $('mhero-file').value = ''; $('mhero-file-lbl').textContent = 'Escolher imagem (JPG/PNG, at\u00e9 12 MB)';
  ...
  var _fileGer = 0;
  function kb(n){ return n >= 1024*1024 ? (n/1024/1024).toFixed(1).replace('.', ',') + ' MB'
                                        : Math.round(n/1024) + ' KB'; }
  async function onFile(e){
    var f = e.target.files && e.target.files[0]; if (!f) return;
    if (!/^image\//.test(f.type)) { $('mhero-msg').textContent = 'Escolha um arquivo de imagem.'; return; }
    if (f.size > 12 * 1024 * 1024) { $('mhero-msg').textContent = 'Imagem acima de 12 MB. Reduza e tente de novo.'; return; }
    var ger = ++_fileGer;
    var original = f.size;
    $('mhero-file-lbl').textContent = f.name + ' (' + kb(original) + ') \u2014 preparando\u2026';
    $('mhero-msg').textContent = '';

    /* Comprime com a mesma funcao ja usada na capa dos cards (05-manual-pce.js),
       com dimensao maior porque o hero ocupa a largura inteira da tela.
       Se falhar, segue com o arquivo original: nunca fica pior do que antes. */
    var usar = f;
    try{
      if (typeof manualCompressImage === 'function' && typeof dataURLtoFile === 'function'){
        var dataUrl = await manualCompressImage(f, 1600, 0.82);
        var cand = dataURLtoFile(dataUrl, 'hero.' + ((dataUrl.indexOf('data:image/webp') === 0) ? 'webp' : 'jpg'));
        if (cand && cand.size > 0 && cand.size < original) usar = cand;
      }
    }catch(err){
      if (window.console) console.warn('[HERO] compressao falhou, usando original', err);
    }
    if (ger !== _fileGer) return;   /* usuario trocou de arquivo no meio */

    if (DRAFT.url && DRAFT.url.indexOf('blob:') === 0) URL.revokeObjectURL(DRAFT.url);
    DRAFT.file = usar; DRAFT.url = URL.createObjectURL(usar);
    $('mhero-file-lbl').textContent = (usar === f)
      ? f.name + ' (' + kb(original) + ')'
      : f.name + ' (' + kb(original) + ' \u2192 ' + kb(usar.size) + ')';
    renderPrev();
  }
  ...
  async function save(){
    ...
    try{
      if (DRAFT.file){
        /* o arquivo aqui ja e o comprimido (webp ou jpg); a extensao sai do
           tipo real, nao do nome original, para o Storage servir certo */
        var ext = (String(DRAFT.file.type || '').split('/')[1] || '').toLowerCase().replace(/[^a-z0-9]/g,'')
               || (DRAFT.file.name.split('.').pop() || 'jpg').toLowerCase().replace(/[^a-z0-9]/g,'') || 'jpg';
